...
public class HopperSim {
    ...
    private Pose3d[] fuelPosesDeployed = new Pose3d[fuelPosesDeployedDimension[0] * fuelPosesDeployedDimension[1] * fuelPosesDeployedDimension[2]];
    private Pose3d[] fuelPosesRetracted = new Pose3d[fuelPosesRetractedDimension[0] * fuelPosesRetractedDimension[1] * fuelPosesRetractedDimension[2]];

    private DoublePublisher hopperLoadPublisher = NetworkTableInstance.getDefault().getDoubleTopic("Sim/Hopper/Current Load").publish();
    private StructArrayPublisher<Pose3d> fuelPosePublisher 
                = NetworkTableInstance.getDefault().getStructArrayTopic("Sim/Hopper/Fuel Poses", Pose3d.struct).publish();
    ...
    public void publishHopperFuelPoses() {
        ...
        for (int i = 0; i < fuelPoses.length; i++) {
            ...
            Pose3d worldFuelPose = localFuelPose.transformBy(robotPoseTransform)
                                    .rotateAround(robotPoseTransform.getTranslation(),new Rotation3d(robotPoseSupplier.get().getRotation()));
